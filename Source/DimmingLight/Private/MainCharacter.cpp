...
void AMainCharacter::BeginPlay() {
	...
	InitialLocation = LightSourceMesh->GetRelativeLocation();
}
...
void AMainCharacter::Tick(float DeltaTime) {
	...
	if (LightSourceMesh) {
		...
		float OffsetX = FMath::Sin(MovementTime * OscilatingSpeed) * MovementOffset.X;
		float OffsetY = FMath::Sin(MovementTime * OscilatingSpeed) * MovementOffset.Y;
		float OffsetZ = FMath::Sin(MovementTime * OscilatingSpeed) * MovementOffset.Z;
		FVector NewLocation = InitialLocation + FVector(OffsetX, OffsetY, OffsetZ);
...
UParticleSystemComponent* AMainCharacter::PlayParticleSystem(UParticleSystem* Effect, FName SocketName) {
	if (!Effect) {
...
// DAMAGEABLE
void AMainCharacter::ReceiveDamage_Implementation(float RawDamage, EDamageType DamageType) {
...
void AMainCharacter::SpecialAttackStart(const FInputActionValue& Value) {
	...
	SpecialAttackArmTrail = PlayParticleSystem(MarkTargetingUpperFX, ArmTrailSocket);
	SpecialAttackHandTrail = PlayParticleSystem(MarkTargetingLowerFX, HandTrailSocket);
...
void AMainCharacter::SpecialAttackRelease(const FInputActionValue& Value) {
	...
	if (SpecialAttackArmTrail) SpecialAttackArmTrail->Deactivate();
	if (SpecialAttackHandTrail) SpecialAttackHandTrail->Deactivate();
